...
export async function selectProfileByProfileEmail(profileEmail: string): Promise<Profile|null>  {
    try {
        ...
        const sqlQuery: string = "SELECT BIN_TO_UUID(profileId) as profileId, profileActivationToken, profileEmail, profileHash, profileFullName FROM profile WHERE profileEmail = :profileEmail"
