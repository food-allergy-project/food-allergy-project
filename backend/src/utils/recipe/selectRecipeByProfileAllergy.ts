...
export async function selectRecipeByProfileAllergy(profileId:string) : Promise<Recipe[]> {
    ...
    const mysqlQuery = 'select BIN_TO_UUID(recipeId) as recipeId, BIN_TO_UUID(recipeProfileId) as recipeProfileId, recipeCategory, recipeDate, recipeIngredients, recipeImage, recipeImageAlt, recipeInstructions, recipeTitle from recipe inner join recipeAllergy on recipe.recipeId = recipeAllergy.recipeAllergyRecipeId where recipeAllergy.recipeAllergyAllergyId in((select profileAllergyAllergyId from profileAllergy where profileAllergyProfileId = UUID_TO_BIN(:profileId)))'
